...
const Button = ({title, value, num, handleClick, activeButton, color}) => {
    ...
    return (
        <button style={activeButton === title && notMobile ? {backgroundColor: color} : activeButton === title && !notMobile ? {borderBottom: `2px solid ${color}`, color: "#ffffff"} : {} } onClick={() => handleClick(title, value)} className={`info-button`}>
            {notMobile ? 
                <>
                    <span>0{num}</span>
                    {" " + fullTitle(title)}
